package RoboBot::Type::Expression;
...
sub evaluate {
    ...
    return unless defined $message && $message->isa('RoboBot::Message');
...
sub function {
    ...
    if (defined $fn) {
        return unless ref($fn) eq 'RoboBot::Type::Function';

        if ($self->has_value) {
            if (defined $self->value->[0] && ref($self->value->[0]) eq 'RoboBot::Type::Function') {
                $self->value->[0] = $fn;
            } else {
                push(@{$self->value}, $fn);
            }
...
sub has_function {
    ...
    return 1
        ...
        && ref($self->value->[0]) eq 'RoboBot::Type::Function';
    return 0;
}
